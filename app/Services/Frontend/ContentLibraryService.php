    ...
    public function getListingPayload(User $user, array $filters): array
    {
        ...
        $query = OrderItem::query()
            ->with([
                ...
                'influencer.user:id,name,profile_image_path'
            ])
            ->when($perspective === 'brand', fn($q) => $q->whereHas('order', fn($oq) => $oq->where('brand_id', $user->brand?->id ?? 0)))
            ->when($perspective === 'influencer', fn($q) => $q->where('influencer_id', $user->influencer?->id ?? 0));
        ...
        if (!empty($filters['search'])) {
            $query->where(function ($q) use ($filters, $perspective) {
                ...
                if ($perspective === 'brand') {
                    $q->orWhereHas('influencer.user', fn($uq) => $uq->where('name', 'like', '%' . $filters['search'] . '%'));
                }

                if ($perspective === 'influencer') {
