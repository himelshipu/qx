{{-- Frontend Campaigns Create/Edit --}}
@extends('frontend.layouts.app')

@section('content')
	<div class="container mx-auto px-4 py-8 max-w-2xl">
		<h1 class="text-3xl font-bold mb-8">{{ isset($campaign) ? 'Edit Campaign' : 'Create Campaign' }}</h1>
		...
		<form
			action="{{ isset($campaign) ? route('frontend.campaigns.update', $campaign) : route('frontend.campaigns.store') }}"
			...
			<div class="pt-6 flex gap-4">
				...
				<a href="{{ route('frontend.campaigns.index') }}" class="btn btn-outline">Cancel</a>
			</div>
		</form>
	</div>
@endsection
