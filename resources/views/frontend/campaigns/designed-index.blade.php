...
@section('content')
	<div x-data="{ showFilters: {{ $status !== 'all' || ($type ?? 'all') !== 'all' ? 'true' : 'false' }} }" class="mt-8 px-2">
		<div class="mb-6">
			@include('backend.pages.campaigns._alerts')
		</div>

		<form method="GET" action="{{ route('frontend.campaigns.index') }}" class="space-y-4">
		...
		<div class="mt-6 grid grid-cols-1 gap-6 pb-20 md:grid-cols-2 lg:grid-cols-3 lg:gap-8">
			@forelse ($campaigns as $campaign)
				...
				<div
					...
					<a href="{{ route('frontend.campaigns.show', $campaign) }}" class="absolute inset-0 z-10"
					...
					<div class="absolute right-5 top-5 z-20 flex items-center gap-2">
						...
						@if ($userType === 'brand' && $campaign->created_by === auth()->user()->id)
							<a href="{{ route('frontend.campaigns.edit', $campaign) }}"
								class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/15 text-white backdrop-blur transition hover:bg-white/25"
								title="Edit campaign">
								<x-icons.edit class="h-4 w-4" />
							</a>
							<form action="{{ route('frontend.campaigns.destroy', $campaign) }}" method="POST" class="relative z-20">
								@csrf
								@method('DELETE')
