...
@section('content')
    ...
    paymentStoreUrl: @js(route('payment-methods.store')),
    csrfToken: @js(csrf_token())
		...
		<div>
			...
			<div x-show="tab === 'details'" x-cloak class="space-y-8 animate-in fade-in duration-300">
				<form action="{{ route('frontend.account.details.update', ['slug' => $user->slug]) }}" method="POST"
					class="space-y-8">
					@csrf
					...
					<div class="space-y-6">
						<!-- Name -->
						<div>
							<label class="block text-sm font-medium text-gray-800 dark:text-gray-300 mb-2">Full Name <span
						...
						<!-- Email -->
						<div>
							<label class="block text-sm font-medium text-gray-800 dark:text-gray-300 mb-2">Email Address <span
									class="text-error-500"> *</span></label>
					...
					<button type="submit"
						class="bg-[#222] shadow-theme-xs h-14 hover:bg-purple-400 flex w-full items-center justify-center rounded-lg px-4 py-3 text-sm font-medium text-white transition">
						Save Changes
					</button>
				</form>
			</div>
			...
			@if ($canManageBillingAndPayment)
				...
				<div x-show="tab === 'billing'" x-cloak class="space-y-8 animate-in fade-in duration-300">
					<form action="{{ route('frontend.account.billing.update', ['slug' => $user->slug]) }}" method="POST"
						class="space-y-8">
						@csrf
						<input type="hidden" name="tab" :value="tab">
						...
						<div class="space-y-6">
							...
							<div>
								...
								<input type="text" name="legal_company_name"
									value="{{ old('legal_company_name', $billingProfile?->legal_company_name ?? '') }}"
									placeholder="Company Name for Invoicing"
									class="dark:bg-dark-900 shadow-theme-xs focus:border-pink-50 focus:ring-gray-500/10 dark:focus:border-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-1 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
							...
							<div>
								...
								<input type="text" name="vat_id" value="{{ old('vat_id', $billingProfile?->vat_id ?? '') }}"
							...
							<div>
								...
								<input type="text" name="billing_address"
									value="{{ old('billing_address', $billingProfile?->billing_address ?? '') }}" placeholder="Street address"
									class="dark:bg-dark-900 shadow-theme-xs focus:border-pink-50 focus:ring-gray-500/10 dark:focus:border-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-1 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
							...
							<div>
								...
								<input type="text" name="billing_city"
									value="{{ old('billing_city', $billingProfile?->billing_city ?? '') }}" placeholder="City Name"
									class="dark:bg-dark-900 shadow-theme-xs focus:border-pink-50 focus:ring-gray-500/10 dark:focus:border-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-1 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
								@error('billing_city')
									<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
								@enderror
							...
							<div>
								...
								<input type="text" name="billing_country"
									value="{{ old('billing_country', $billingProfile?->billing_country ?? '') }}" placeholder="Country"
									class="dark:bg-dark-900 shadow-theme-xs focus:border-pink-50 focus:ring-gray-500/10 dark:focus:border-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-1 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
							...
							<div>
								...
								<input type="text" name="billing_postal_code"
									value="{{ old('billing_postal_code', $billingProfile?->billing_postal_code ?? '') }}"
									placeholder="The postal or ZIP code for your billing address"
									class="dark:bg-dark-900 shadow-theme-xs focus:border-pink-50 focus:ring-gray-500/10 dark:focus:border-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-1 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
			...
			<div x-show="tab === 'password'" x-cloak class="space-y-8 animate-in fade-in duration-300" x-data="{ showPass: { old: false, new: false, confirm: false } }">
				<form action="{{ route('frontend.account.password.update', ['slug' => $user->slug]) }}" method="POST"
					class="space-y-6">
					@csrf
					<input type="hidden" name="tab" :value="tab">

					<div>
						<label class="block text-sm font-medium text-gray-800 dark:text-gray-300 mb-2">Current Password <span
								class="text-error-500"> *</span></label>
						<div class="relative">
							<input :type="showPass.old ? 'text' : 'password'" name="current_password"
					...
					<div>
						<label class="block text-sm font-medium text-gray-800 dark:text-gray-300 mb-2">New Password <span
								class="text-error-500"> *</span></label>
						<div class="relative">
						...
						<p class="text-xs text-gray-500 dark:text-gray-400 mt-2">At least 8 characters with uppercase, lowercase, number,
							and symbol</p>
						@error('password')
							<p class="mt-1 text-xs text-red-500">{{ $message }}</p>
						@enderror
					...
					<div>
						<label class="block text-sm font-medium text-gray-800 dark:text-gray-300 mb-2">Confirm Password <span
								class="text-error-500"> *</span></label>
						<div class="relative">
							<input :type="showPass.confirm ? 'text' : 'password'" name="password_confirmation"
			...
			<div x-show="tab === 'security'" x-cloak class="space-y-12 animate-in fade-in duration-300">
				...
				<div class="border border-gray-200 dark:border-gray-800 rounded-xl p-6">
					<div class="flex items-center justify-between flex-wrap gap-4">
						...
						<form action="{{ route('frontend.account.toggle-status', ['slug' => $user->slug]) }}" method="POST">
							@csrf
							<input type="hidden" name="tab" value="security">
							<button type="submit"
								class="px-4 py-2 rounded-lg font-medium text-sm transition {{ $user->is_active ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }}">
				...
				<div class="border border-gray-200 dark:border-gray-800 rounded-xl p-6">
					<div class="flex items-center justify-between flex-wrap gap-4">
						...
						<form method="POST" action="{{ route('logout') }}">
							@csrf
							<button type="submit"
								class="px-6 py-2 rounded-lg font-medium text-sm bg-yellow-100 text-yellow-700 hover:bg-yellow-200 transition">
								Sign Out
							</button>
						</form>
					</div>
				...
				<!-- Delete Account -->
				<div class="border-2 border-red-200 dark:border-red-900 rounded-xl p-6 bg-red-50 dark:bg-red-950/20"
					...
					<div class="flex items-center justify-between mb-4 flex-wrap gap-4">
						...
						<button @click="deleteOpen = !deleteOpen"
							class="px-4 py-2 rounded-lg font-medium text-sm bg-red-100 text-red-700 hover:bg-red-200 transition">
							Delete
						</button>
					</div>

					<div x-show="deleteOpen" x-cloak
						class="mt-4 p-4 bg-white dark:bg-gray-900 rounded-lg border border-red-200 dark:border-red-800">
						...
						<form method="POST" action="{{ route('dashboard.account.destroy', ['slug' => $user->slug]) }}"
							class="js-confirmable" data-confirm-title="Delete Account"
							data-confirm-message="Are you absolutely sure? All your data will be permanently deleted."
							data-confirm-button="Delete Account" data-confirm-variant="danger">
							@csrf
							@method('DELETE')
							<input type="hidden" name="tab" value="security">
							<div class="flex gap-3 flex-wrap">
								<input type="password" name="password" placeholder="Enter your password"
									...
									required>
								<button type="submit"
									class="px-6 py-2 rounded-lg font-medium text-sm bg-red-500 text-white hover:bg-red-600 transition active:scale-95">
									Delete Account
								</button>
							</div>
							@error('password')
			...
			@if ($canManageBillingAndPayment)
				<!-- Payment Tab -->
				<div x-show="tab === 'payment'" x-cloak class="space-y-8 animate-in fade-in duration-300">
						...
						@if ($paymentMethods && $paymentMethods->count() > 0)
							<div class="space-y-3">
								@foreach ($paymentMethods as $method)
									<div
										class="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-800 rounded-lg bg-gray-50 dark:bg-gray-900/50 hover:border-gray-300 dark:hover:border-gray-700 transition">
											...
											<div class="flex-1">
												<div class="flex items-center gap-2 flex-wrap">
													...
													@if ($method->is_default)
														<span
															class="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-medium rounded">
															Default
														</span>
													@endif
													@if ($method->isExpired())
														<span
															class="px-2 py-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs font-medium rounded">
															Expired
														</span>
													@elseif($method->isExpiringSoon())
														<span
															class="px-2 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 text-xs font-medium rounded">
															Expiring Soon
														</span>
													@endif
												</div>
										...
										<div class="flex items-center gap-2">
											...
											<form action="{{ route('payment-methods.destroy', $method->id) }}" method="POST"
												...
												<button type="submit"
													...
													data-confirm-message="Are you sure you want to delete this card?" data-confirm-button="Delete"
