...
<section class="min-h-screen" 
    x-data="{ 
        ...
        price: '$200',
        packages: [
            { name: '1 Instagram Photo Feed Post', icon: 'instagram' },
            { name: '1 Instagram Reel (60 Seconds)', icon: 'instagram' },
            { name: '1 Instagram Story', icon: 'instagram' },
            { name: '1 Instagram Live (60 Seconds)', icon: 'instagram' },
            { name: '1 TikTok Video (60 Seconds)', icon: 'tiktok' },
            { name: '1 TikTok Story', icon: 'tiktok' },
            { name: '1 TikTok Live (60 Seconds)', icon: 'tiktok' },
            { name: '1 UGC Product Video (60 Seconds)', icon: 'camera' },
            { name: '1 UGC Product Photo', icon: 'camera' },
            { name: '1 UGC Video Ad (60 Seconds)', icon: 'camera' },
            { name: '1 UGC Photo Ad', icon: 'camera' },
            { name: '1 UGC Tutorial (60 Seconds)', icon: 'camera' },
            { name: '1 UGC Testimonial/Review (60 Seconds)', icon: 'camera' },
            { name: '1 UGC Unboxing (60 Seconds)', icon: 'camera' },
            { name: '1 UGC Blog (60 Seconds)', icon: 'camera' }
        ]
    }">
    ...
    <main class="max-w-screen-2xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <button class="flex items-center gap-2 px-5 py-2 border border-gray-200 dark:border-gray-800 rounded-lg text-sm font-bold text-[#222] hover:bg-purple-50 transition active:scale-95">
        ...
        <div class="flex flex-col lg:flex-row gap-16">
            ...
            <div class="flex-1 space-y-12">
                ...
                <!-- DESCRIPTION BIO -->
                <div class="text-gray-600 dark:text-gray-300 text-lg max-w-4xl font-normal opacity-90">
                ...
                <div class="sticky top-24 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2.5rem] p-10 shadow-2xl shadow-purple-900/5">
                    ...
                    <div class="relative mb-4">
                        <button @click="openDropdown = !openDropdown" 
                                class="w-full flex items-center justify-between px-5 py-4 border-1 border-gray-400 dark:border-gray-500 rounded-xl text-base font-bold text-gray-800 dark:text-gray-300 bg-white dark:bg-transparent transition-all hover:shadow-lg">
                            <div class="flex items-center gap-4">
                                <template x-if="selectedPackage.includes('Instagram')"><x-icons.instagram class="w-5 h-5 text-[#28303F] dark:text-white" /></template>
                                <template x-if="selectedPackage.includes('TikTok')"><x-icons.tiktok class="w-5 h-5 text-[#28303F] dark:text-white" /></template>
                        ...
                        <!-- Dropdown List -->
                        <div x-show="openDropdown" x-cloak @click.away="openDropdown = false" class="absolute top-full left-0 w-full mt-0 bg-white dark:bg-gray-800 border-2 border-gray-400 dark:border-gray-500 rounded-b-2xl shadow-md z-50 overflow-y-auto max-h-96">
                            <template x-for="p in packages" :key="p.name">
                                <div @click="selectedPackage = p.name; openDropdown = false" 
                                     class="px-6 py-4 cursor-pointer text-sm font-bold tracking-tight transition-colors"
                                     :class="selectedPackage === p.name ? 'bg-purple-100 text-black' : 'text-gray-800 dark:text-gray-300 hover:bg-purple-50 dark:hover:bg-purple-900/40'"
                                     x-text="p.name">
                                </div>
                            </template>
                        </div>
