        ...
        <section class="py-14 max-w-svw mx-auto">
            ...
            <div class="relative mb-20">
                @if($brand->user->cover_image_path)
                    <img src="{{ \App\Helpers\ImageHelper::url($brand->user->cover_image_path) }}" alt="Cover" class="w-full h-80 md:h-[320px] rounded-3xl object-cover">
            ...
            <div class="text-center">
                ...
                <!-- Social Links -->
                @if(($brand->website) || ($brand->socialLinks && ($brand->socialLinks->instagram_url || $brand->socialLinks->facebook_url || $brand->socialLinks->youtube_url || $brand->socialLinks->tiktok_url || $brand->socialLinks->x_url || $brand->socialLinks->linkedin_url)))
                    <div class="flex flex-wrap gap-3 mt-6 justify-center">
                        @if($brand->website)
                            <a href="{{ $brand->website }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                            <x-icons.website class="w-5 h-5" />
                            </a>
                        @endif
                        @if($brand->socialLinks?->instagram_url)
                            <a href="{{ $brand->socialLinks?->instagram_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                            <x-icons.instagram class="w-5 h-5" />
                            </a>
                        @endif
                        @if($brand->socialLinks?->facebook_url)
                            <a href="{{ $brand->socialLinks?->facebook_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                            <x-icons.facebook class="w-5 h-5" />
                            </a>
                        @endif
                        @if($brand->socialLinks?->youtube_url)
                            <a href="{{ $brand->socialLinks?->youtube_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                                <x-icons.youtube class="w-5 h-5" />
                            </a>
                        @endif
                        @if($brand->socialLinks?->tiktok_url)
                            <a href="{{ $brand->socialLinks?->tiktok_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                                <x-icons.tiktok class="w-5 h-5" />
                            </a>
                        @endif
                        @if($brand->socialLinks?->x_url)
                            <a href="{{ $brand->socialLinks?->x_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                                <x-icons.x class="w-5 h-5" />
                            </a>
                        @endif
                        @if($brand->socialLinks?->linkedin_url)
                            <a href="{{ $brand->socialLinks?->linkedin_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition">
