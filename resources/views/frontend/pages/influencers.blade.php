...
@section('content')
        ...
        <main>
            <section class="w-full flex flex-col gap-8">
                ...
                <div class="w-full md:w-2/3 mx-auto">
                    <div
                        class="bg-white dark:bg-gray-800 w-full rounded-md md:rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row p-5 md:p-2 md:pl-10 relative items-start md:items-center">

                        <!-- Platform selector -->
                        <div class="relative flex-1">
                            <div id="platform-trigger"
                                class="flex flex-col items-start cursor-pointer border-b md:border-b-0 md:border-r border-gray-100 dark:border-gray-700 pb-4 md:pb-0 md:pr-4 group">
                                <span
                                    class="text-[11px] font-bold text-gray-900 dark:text-gray-100 uppercase tracking-wider">Platform</span>
                                <span id="selected-platform"
                                    class="text-sm truncate text-gray-900 dark:text-white">{{ $currentPlatformLabel }}</span>
                            </div>
                            ...
                            <div id="platform-menu"
                                ...
                                <div
                                    ...
                                    data-url="{{ route('influencers') }}" data-value="All Platforms">
                                    All Platforms
                                </div>

                                @foreach ($platformFilters as $platform)
                                    <div class="platform-option px-4 py-3 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl cursor-pointer transition-colors"
                                        data-url="{{ route('influencers', ['platformSlug' => $platform['slug']]) }}"
                                        data-value="{{ $platform['label'] }}">
                                        {{ $platform['label'] }}
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="relative flex-[1.5] flex flex-col items-start pt-4 md:pt-0 md:pl-8 group">
                            <div id="category-trigger" class="w-full cursor-pointer">
                                <span
                                    class="text-[11px] font-bold text-gray-900 dark:text-gray-100 uppercase tracking-wider">Category</span>
                            ...
                            <div id="category-menu"
                                class="hidden absolute top-full left-[24px] right-0 mt-4 w-[90vw] md:w-[600px] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2rem] shadow-2xl z-50 p-6 transition-all">
                        ...
                        <div class="flex justify-end mt-4 md:mt-0 md:items-center w-full md:w-auto">
                            <button type="button"
                                class="bg-[#222] hover:opacity-80 transition-all p-4 md:p-5 rounded-full text-white shadow-lg md:ml-2"
                ...
                <div class="max-w-6xl flex flex-wrap items-center gap-3 justify-center mb-16 mx-auto px-4">
                    ...
                    @foreach ($badges as $badge)
                        <button
                            ...
                            <div
                                class="absolute inset-0 opacity-0 group-hover:opacity-10 bg-gradient-to-r from-[#9333EA] via-[#C084FC] to-[#A94DFF] transition-opacity">
                            </div>

                            <span class="relative z-10">
                                @include('components.icons.' . $badge['icon'], [
                                    'class' => 'w-4 h-4 text-gray-900 dark:text-gray-100',
                                ])
                            </span>
                            <span
                                class="relative z-10 text-sm group-hover:text-black dark:group-hover:text-white">{{ $badge['label'] }}</span>
                        </button>
                    @endforeach
                </div>
            </section>
                ...
                @else
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        @foreach ($influencers as $creator)
                            @php
                                $profileUrl = !empty($creator['slug'])
                                    ? route('creator.profile', ['slug' => $creator['slug']])
                                    : '#';
                            @endphp
    ...
    <div id="wishlist-modal" class="fixed inset-0 z-[100] hidden items-center justify-center p-4">
        ...
        <div class="relative w-auto lg:w-[500px] max-w-md bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-2xl overflow-hidden transform transition-all scale-95 opacity-0 duration-300"
            id="modal-container">
                ...
                <div id="existing-lists" class="space-y-2">
                    <button
                        ...
                        <div class="w-14 h-14 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center overflow-hidden">
                            ...
                            <svg class="placeholder-icon w-6 h-6 text-gray-400" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', (e) => {
                if (platformTrigger && platformMenu && !platformTrigger.contains(e.target) && !platformMenu.contains(e.target)) {
                    platformMenu.classList.add('hidden');
                }

                if (categoryTrigger && categoryMenu && !categoryTrigger.contains(e.target) && !categoryMenu.contains(e.target)) {
                    categoryMenu.classList.add('hidden');
                }
            });
        });
