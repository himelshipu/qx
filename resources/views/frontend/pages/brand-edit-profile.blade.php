...
@section('content')
    ...
    init() {
        ...
        if (storedTab) {
            this.tab = storedTab;
            sessionStorage.removeItem('activeBrandProfileTab');
        }
    },
    ...
    async submitForm(event) {
        ...
        try {
            ...
            // For images tab, reload page to show updated images from server
            if (this.tab === 'images') {
                // Store the active tab in sessionStorage so page reloads to correct tab
                sessionStorage.setItem('activeBrandProfileTab', 'images');
		...
		<div class="max-w-4xl mx-auto">
			<div class="mb-8 text-start">
				<a href="{{ route('brand.profile', ['slug' => $slug]) }}"
			...
			<h1 class="text-3xl md:text-4xl font-semibold text-[#222] dark:text-white leading-tight text-left mb-4">Edit Profile
			</h1>
					...
					: 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
								...
								<button @click="tab = 'images'"
									:class="tab === 'images' ? 'border-b-2 border-black dark:border-white text-black dark:text-white' :
									    'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'"
									class="pb-4 text-base font-medium transition-all whitespace-nowrap">Images</button>
							</div>

						<form action="{{ route('brand.profile.update', ['slug' => $slug]) }}" method="POST"
								...
								<div x-show="tab === 'details'" x-cloak class="space-y-8 text-start animate-in fade-in duration-300">
									<!-- Brand Name -->
									<div>
										<label class="block text-sm font-medium text-gray-800 dark:text-gray-400 mb-2">Brand Name <span
												class="text-red-500">*</span></label>
										<input type="text" name="brand_name" required value="{{ old('brand_name', $brand->brand_name ?? '') }}"
	...
	@enderror
								...
								<div x-show="tab === 'social'" x-cloak class="space-y-8 text-start animate-in fade-in duration-300">
									<div>
										...
										<input type="url" name="website" @blur="normalizeSocialInput($event)"
											value="{{ old('website', $brand->website ?? '') }}" placeholder="https://yourwebsite.com"
											class="dark:bg-dark-900 shadow-theme-xs focus:border-pink-50 focus:ring-gray-500/10 dark:focus:border-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-1 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
	...
	@enderror
									...
									<div>
										...
										<input type="url" name="instagram" @blur="normalizeSocialInput($event)"
											value="{{ old('instagram', $brand?->socialLinks?->instagram_url ?? '') }}"
	...
	@enderror
									...
									<div>
										...
										<input type="url" name="tiktok" @blur="normalizeSocialInput($event)"
											value="{{ old('tiktok', $brand?->socialLinks?->tiktok_url ?? '') }}"
	...
	@enderror
									...
									<div>
										...
										<input type="url" name="facebook" @blur="normalizeSocialInput($event)"
											value="{{ old('facebook', $brand?->socialLinks?->facebook_url ?? '') }}"
	...
	@enderror
									...
									<div>
										...
										<input type="url" name="x" @blur="normalizeSocialInput($event)"
											value="{{ old('x', $brand?->socialLinks?->x_url ?? '') }}" placeholder="https://x.com/yourprofile"
											class="dark:bg-dark-900 shadow-theme-xs focus:border-pink-50 focus:ring-gray-500/10 dark:focus:border-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-1 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
	...
	@enderror
									...
									<div>
										...
										<input type="url" name="youtube" @blur="normalizeSocialInput($event)"
											value="{{ old('youtube', $brand?->socialLinks?->youtube_url ?? '') }}"
	...
	@enderror
									...
									<div>
										...
										<input type="url" name="linkedin" @blur="normalizeSocialInput($event)"
											value="{{ old('linkedin', $brand?->socialLinks?->linkedin_url ?? '') }}"
								...
								<div x-show="tab === 'images'" x-cloak class="space-y-12 text-center animate-in fade-in duration-300">
									...
									<div class="flex flex-col items-center">
										<div class="relative group">
											<div
												class="relative w-32 h-32 rounded-full bg-gradient-to-br from-purple-100 to-blue-100 dark:from-purple-900 dark:to-blue-900 flex items-center justify-center border-4 border-white dark:border-gray-800 shadow-lg overflow-hidden cursor-pointer transition hover:shadow-xl"
												@click="$el.closest('.group').querySelector('input[name=profile_image]').click()">
												<input x-ref="profileInput" type="file" name="profile_image" class="hidden"
													@change="handleProfileUpload($event)" accept="image/*">
	...
	<div class="relative w-full">
		<div class="absolute top-4 left-4 z-20">
			<span
				class="bg-white/90 dark:bg-gray-800 px-3 py-1 rounded-lg text-[11px] font-bold text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 uppercase tracking-wide">Cover
				Photo</span>
		</div>
		...
		<div @click="$el.querySelector('input[name=cover_image]').click()"
			...
			<input x-ref="coverInput" type="file" name="cover_image" class="hidden" @change="handleCoverUpload($event)"
				accept="image/*">
	...
	<div class="flex justify-end gap-4 mt-16 pt-8 border-t border-gray-200 dark:border-gray-800">
		<a href="{{ url('/dashboard') }}"
			class="px-6 py-3 rounded-lg font-medium text-sm border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
			Cancel
		</a>
		<button type="submit" :disabled="isSaving"
			class="bg-[#222] shadow-theme-xs hover:bg-purple-400 flex items-center justify-center rounded-lg px-4 py-3 text-sm font-medium text-white transitionactive:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">
