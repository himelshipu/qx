...
<section class="min-h-screen transition-colors duration-200">
    ...
    <form method="GET" action="{{ route('dashboard.content-library') }}" class="flex flex-wrap items-center gap-4 mb-10" id="filters-form">
        <div class="relative">
            ...
            <x-icons.chevron-right class="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 rotate-90 text-gray-500 dark:text-gray-400" />
        </div>

        <div class="relative">
            <select name="status" id="status-filter" class="appearance-none h-12 w-[220px] pr-10 px-4 rounded-lg border border-gray-300 bg-transparent text-sm font-bold text-gray-800 transition focus:border-gray-400 focus:ring-1 focus:ring-gray-400/20 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-gray-500 dark:focus:ring-gray-500/20">
        ...
        <div class="relative">
            <x-icons.clock class="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-500 dark:text-gray-400" />
            <input
                type="text"
                id="date-range-display"
                placeholder="Date: Select Range"
        ...
        <div class="ml-auto flex items-center gap-2">
            <input type="text" name="q" id="search-input" value="{{ $filters['search'] }}" placeholder="Search anything in library..." class="h-12 w-64 rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 transition focus:border-gray-400 focus:ring-1 focus:ring-gray-400/20 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-gray-500 dark:focus:ring-gray-500/20">
            <a href="{{ route('dashboard.content-library') }}" class="h-12 inline-flex items-center px-5 border border-gray-300 rounded-lg text-[13px] font-bold text-gray-800 dark:text-white hover:bg-purple-50 transition">Reset</a>
        </div>
    </form>

    <div id="content-library-results" class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03] shadow-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse ($entries as $entry)
                        @php
                            ...
                            $statusClass = $statusClasses[$entry->order?->status ?? 'pending'] ?? 'bg-orange-50 text-orange-600 border border-orange-100 dark:bg-yellow-500/15 dark:text-orange-400';
                        @endphp
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('dashboard.orders.show', $entry->order_id) }}" class="text-gray-400 hover:text-purple-400 transition-colors">
        ...
        @if ($entries->hasPages())
            ...
            <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex justify-between items-center">
                ...
                @else
                    <a href="{{ $entries->appends(request()->query())->previousPageUrl() }}" class="js-library-page-link text-sm font-bold text-gray-500 hover:text-purple-400 transition">Previous</a>
                @endif

                <div class="flex gap-2">
                    @for ($page = $startPage; $page <= $endPage; $page++)
                        <a
                            href="{{ $entries->appends(request()->query())->url($page) }}"
                ...
                @if ($entries->hasMorePages())
                    <a href="{{ $entries->appends(request()->query())->nextPageUrl() }}" class="js-library-page-link text-sm font-bold text-gray-500 hover:text-purple-400 transition">Next</a>
                @else
