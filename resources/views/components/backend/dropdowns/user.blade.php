<div class="relative" x-data="{ open: false }" @click.away="open = false">
    ...
    <button @click="open = !open" class="flex items-center gap-2 text-gray-700 dark:text-gray-400">
        ...
        @else
            <div class="w-10 h-10 rounded-full bg-[#FFE4C4] flex items-center justify-center text-sm font-bold text-black uppercase">
                {{ $initials }}
            </div>
        @endif
    ...
    <div x-show="open" x-transition
        ...
        <div class="py-1">
            @if ($user->brand)
                ...
                <a href="{{ route('dashboard.brand.profile.edit', ['slug' => $user->slug]) }}"
                    class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
            ...
            @elseif($user->creator)
                ...
                <a href="{{ route('dashboard.creator.profile.edit',['slug' => $user->slug]) }}"
                    class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
                    <x-icons.edit class="w-5 h-5" />
                    Edit profile
                </a>
