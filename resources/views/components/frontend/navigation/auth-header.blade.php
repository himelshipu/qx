		...
		<div class="mx-auto py-6 flex flex-col sm:px-0 px-4 sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
			...
			<div class="flex items-center gap-5">
				...
				<div class="relative group">
					<button
						class="flex items-center gap-3 border border-gray-100 rounded-full p-1 pl-4 bg-white hover:shadow-md transition-all duration-300 active:scale-95">
						...
						@auth
							@if (Auth::user()->profile_image_path && file_exists(public_path(Auth::user()->profile_image_path)))
								<img src="{{ asset(Auth::user()->profile_image_path) }}" alt="Profile"
					...
					<div
						class="absolute right-0 top-full mt-3 w-56 bg-white rounded-[20px] shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-gray-50 invisible opacity-0 scale-95 group-hover:visible group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 origin-top-right z-50 overflow-hidden">
						<div class="py-2 flex flex-col">
							@auth
								@if (Auth::user()->brand)
									@php $b = Auth::user()->brand; @endphp
									<a href="{{ route('brand.profile', ['id' => $b->id]) }}"
										class="px-7 py-3.5 text-[15px] font-bold text-gray-800 hover:bg-gray-50 transition-colors">View profile</a>
									<a href="{{ route('dashboard.brand.profile.edit') }}"
										class="px-7 py-3.5 text-[15px] font-medium text-gray-600 hover:bg-gray-50 transition-colors">Edit profile</a>
								@elseif(Auth::user()->creator)
									<a href="{{ route('creator.profile', ['slug' => Auth::user()->slug]) }}"
										class="px-7 py-3.5 text-[15px] font-bold text-gray-800 hover:bg-gray-50 transition-colors">View profile</a>
									<a href="{{ route('dashboard.creator.profile.edit') }}"
										class="px-7 py-3.5 text-[15px] font-medium text-gray-600 hover:bg-gray-50 transition-colors">Edit profile</a>
								@else
									<a href="{{ route('dashboard.account.edit') }}"
										class="px-7 py-3.5 text-[15px] font-bold text-gray-800 hover:bg-gray-50 transition-colors">Profile</a>
								@endif
							@endauth

							<a href="#"
								class="px-7 py-3.5 text-[15px] font-bold text-gray-800 hover:bg-gray-50 transition-colors">Offers</a>

							<div class="border-t border-gray-100 my-1 mx-2"></div>

							<a href="{{ route('dashboard.account.edit') }}"
								class="px-7 py-3.5 text-[15px] font-medium text-gray-600 hover:bg-gray-50 transition-colors">
								Account
							</a>
				...
				<div x-show="isCartOpen" x-transition:enter="transform transition ease-in-out duration-500"
					...
					<div class="hidden md:flex flex-col w-[38%] bg-black p-12 text-white justify-between">
						...
						<div class="flex items-start gap-4">
							...
							<p class="text-[11px] font-bold text-gray-500 uppercase tracking-tight leading-4">
								...
								<span class="text-gray-600 font-medium normal-case tracking-normal">If an order is declined, funds will be
									refunded.</span>
							</p>
						</div>
					</div>
					...
					<div class="flex-1 flex flex-col bg-white p-12 justify-between">
						...
						<button @click="isCartOpen = false"
							class="w-full bg-[#1A1A1A] text-white py-5 rounded-2xl font-bold text-sm tracking-widest hover:bg-black transition-all shadow-xl active:scale-95">
