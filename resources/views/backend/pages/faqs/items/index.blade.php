						...
						@forelse($items as $item)
							<tr>
								...
								<td class="px-4 py-3 text-sm">
									<span
										class="inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium {{ $item->is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400' }}">
										<span class="h-1.5 w-1.5 rounded-full {{ $item->is_active ? 'bg-green-500' : 'bg-gray-500' }}"></span>
								...
								<td class="px-4 py-3 text-right">
									<div class="flex items-center justify-end gap-2">
										<form action="{{ route('dashboard.faqs.items.toggle-status', [$section, $item]) }}" method="POST"
											class="inline">
											@csrf
											<button type="submit"
