...
@section('content')
	...
	<div class="space-y-6" id="influencers-dashboard"
		data-csrf-token="{{ csrf_token() }}"
		data-filter-results-route="{{ route('dashboard.influencers.table') }}"
		data-status-toggle-template="{{ route('dashboard.influencers.toggle-status', ['influencer' => '__ID__']) }}"
		data-max-featured="{{ (int) config('influencer.max_featured', 20) }}"
		data-featured-add-template="{{ route('dashboard.influencers-featured.add', ['influencer' => '__ID__']) }}"
		data-featured-remove-template="{{ route('dashboard.influencers-featured.remove', ['influencer' => '__ID__']) }}"
		data-featured-list-route="{{ route('dashboard.influencers-featured.list') }}"
		data-featured-search-route="{{ route('dashboard.influencers-featured.search') }}"
		data-featured-reorder-route="{{ route('dashboard.influencers-featured.reorder') }}">
		...
		<div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
			<div class="flex flex-col gap-4 border-b border-gray-200 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
				<div>
					...
					<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage influencer accounts, categories, status, and ordering from one view.</p>
				</div>
			...
			<div class="p-5">
				...
				@include('backend.pages.influencers._results', ['influencers' => $influencers])
			</div>
		</div>
	</div>
		...
		<div class="flex min-h-screen items-center justify-center p-4">
			<div class="relative flex max-h-[60vh] w-full max-w-4xl flex-col overflow-hidden rounded-lg bg-white shadow-xl dark:bg-gray-900">
				<div class="flex items-center justify-between border-b border-gray-200 p-6 dark:border-gray-800">
					<h2 class="text-xl font-semibold text-gray-900 dark:text-white">Manage Featured Influencers</h2>
				...
				<div class="flex-1 overflow-y-auto p-6">
					<div class="mb-6">
						<div class="relative">
							...
							<input id="featured-search" type="text" placeholder="Search influencers to add..."
								class="h-10 w-full rounded-lg border border-gray-200 bg-transparent pl-10 pr-3 text-sm text-gray-900 focus:border-gray-400 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">
						</div>
					...
					<div class="mb-4">
						<h3 class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
							Featured Influencers (<span id="featured-count">0</span>/{{ (int) config('influencer.max_featured', 20) }})
						</h3>
