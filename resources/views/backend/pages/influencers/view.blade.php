...
@section('content')
	@php
		$profileUrl = $influencer->user?->profile_image_path 
			? \App\Helpers\ImageHelper::url($influencer->user->profile_image_path)
			: null;
		$coverUrl = $influencer->user?->cover_image_path 
			? \App\Helpers\ImageHelper::url($influencer->user->cover_image_path)
			: null;
	@endphp
	...
	<div class="space-y-6">
		<div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
			<div class="relative h-44 w-full bg-gray-100 dark:bg-gray-800">
				@if ($coverUrl)
					<img src="{{ $coverUrl }}" alt="{{ $influencer->display_name ?: $influencer->user?->name }} cover" class="h-full w-full object-cover">
				@else
				...
				<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
					<div class="flex items-center gap-4">
						<div class="-mt-14 flex h-20 w-20 items-center justify-center overflow-hidden rounded-full border-4 border-white bg-gray-100 shadow-sm dark:border-gray-900 dark:bg-gray-800">
							@if ($profileUrl)
								<img src="{{ $profileUrl }}" alt="{{ $influencer->display_name ?: $influencer->user?->name }} profile" class="h-20 w-20 object-cover">
							@else
								<span class="text-xl font-semibold text-gray-500 dark:text-gray-300">{{ strtoupper(substr($influencer->display_name ?: $influencer->user?->name ?? 'C', 0, 1)) }}</span>
							@endif
						</div>
						<div>
							<h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ $influencer->display_name ?: $influencer->user?->name ?? 'Unnamed' }}</h2>
							<p class="text-sm text-gray-500 dark:text-gray-400">{{ $influencer->title_name ?: 'No title set' }}</p>
							<div class="mt-2 flex flex-wrap items-center gap-2">
								<span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $influencer->is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' }}">
								...
								@if ($influencer->is_featured)
									<span class="inline-flex rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">
										Featured{{ $influencer->featured_priority ? ' #' . $influencer->featured_priority : '' }}
									</span>
								@endif
							</div>
						</div>
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
			<div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
				...
				<p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ $influencer->campaign_applications_count }}</p>
			</div>
			<div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
				<p class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Order Items</p>
				<p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ $influencer->order_items_count }}</p>
			</div>
			<div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
				<p class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Cart Items</p>
				<p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ $influencer->cart_items_count }}</p>
			</div>
			<div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
				<p class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Conversations</p>
				<p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ $influencer->conversations_count }}</p>
			</div>
		</div>
			...
			<div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
				...
				<dl class="mt-4 space-y-3 text-sm">
					<div class="flex justify-between gap-4">
						...
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->name ?? 'N/A' }}</dd>
					</div>
					<div class="flex justify-between gap-4">
						<dt class="text-gray-500 dark:text-gray-400">Email</dt>
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->email ?? 'N/A' }}</dd>
					</div>
					<div class="flex justify-between gap-4">
						<dt class="text-gray-500 dark:text-gray-400">Phone</dt>
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->phone ?: 'N/A' }}</dd>
					</div>
					<div class="flex justify-between gap-4">
						<dt class="text-gray-500 dark:text-gray-400">Gender</dt>
						<dd class="font-medium text-gray-900 dark:text-white">{{ ucfirst($influencer->user?->gender ?? 'n/a') }}</dd>
					</div>
					<div class="flex justify-between gap-4">
						<dt class="text-gray-500 dark:text-gray-400">Joined</dt>
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->created_at?->format('M d, Y') ?? 'N/A' }}</dd>
					</div>
				</dl>
			</div>
			...
			<div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
				...
				<dl class="mt-4 space-y-3 text-sm">
					<div class="flex justify-between gap-4">
						...
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->address_line ?: 'N/A' }}</dd>
					</div>
					<div class="flex justify-between gap-4">
						<dt class="text-gray-500 dark:text-gray-400">City</dt>
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->city ?: 'N/A' }}</dd>
					</div>
					<div class="flex justify-between gap-4">
						<dt class="text-gray-500 dark:text-gray-400">Country</dt>
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->country ?: 'N/A' }}</dd>
					</div>
					<div class="flex justify-between gap-4">
						<dt class="text-gray-500 dark:text-gray-400">Postal Code</dt>
						<dd class="font-medium text-gray-900 dark:text-white">{{ $influencer->user?->postal_code ?: 'N/A' }}</dd>
					</div>
				</dl>
			</div>
		</div>
		...
		<div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
			...
			<p class="mt-3 whitespace-pre-line text-sm leading-6 text-gray-700 dark:text-gray-300">{{ $influencer->user?->bio ?: 'No bio provided.' }}
			</p>
			...
			<p class="mt-3 whitespace-pre-line text-sm leading-6 text-gray-700 dark:text-gray-300">
				{{ $influencer->audience ?: 'No audience summary provided.' }}</p>
		</div>
	</div>
@endsection
