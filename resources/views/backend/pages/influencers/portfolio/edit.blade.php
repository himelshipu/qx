...
@section('title', "Edit Portfolio - {$influencer->display_name}")
...
@section('content')
	...
	<div class="max-w-2xl">
		<div class="space-y-6">
			<div>
				<h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Portfolio Item</h1>
				<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update this portfolio item for {{ $influencer->display_name }}
				</p>
			</div>

			<form action="{{ route('dashboard.influencers.portfolio.update', [$influencer, $portfolio]) }}" method="POST"
				...
				<div class="space-y-6 p-5">
					...
					<div>
						...
						<div class="space-y-3">
							<label class="flex items-center">
								<input type="radio" name="media_type" value="image" {{ $portfolio->media_type === 'image' ? 'checked' : '' }}
							...
							<label class="flex items-center">
								<input type="radio" name="media_type" value="video" {{ $portfolio->media_type === 'video' ? 'checked' : '' }}
									class="h-4 w-4 text-purple-600 dark:text-purple-500">
					...
					<div>
						...
						<div
							...
							<div class="text-center pointer-events-none">
								...
								<p class="text-xs text-gray-500 dark:text-gray-500 mt-1">Leave empty to keep current file</p>
							</div>
						</div>
						@error('file')
							<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
					...
					<div id="preview-container" class="hidden">
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">New Preview</label>
					...
					<div>
						...
						<textarea id="description" name="description" rows="4" maxlength="1000" placeholder="Describe this media item..."
						 class="block w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm text-gray-900 placeholder-gray-500 focus:border-purple-500 focus:ring-purple-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder-gray-400 resize-none">{{ old('description', $portfolio->description) }}</textarea>
						@error('description')
							<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
						@enderror
					</div>
