...
@section('content')
	<x-backend.shell.breadcrumb pageTitle="Edit Package" />
	...
	<div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
		<div
			...
			<div>
				<h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit: {{ $package->name }}</h3>
				<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update package platform, pricing, delivery settings, and
					availability.</p>
			</div>
		...
		<form action="{{ route('dashboard.packages.update', $package) }}" method="POST" novalidate class="space-y-6 p-5">
			@csrf
			@method('PUT')

			@include('backend.pages.packages._form', ['package' => $package])
