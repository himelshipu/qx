...
@section('content')
	@php
		...
		$sidebarConversations = match ($user->user_type) {
		    'brand' => \App\Models\Conversation::query()->where('brand_user_id', $user->id),
		    'moderator' => \App\Models\Conversation::query()->where('handled_by_user_id', $user->id),
		    default => \App\Models\Conversation::query(),
		};

		$sidebarConversations = $sidebarConversations
		    ->with(['influencer.user', 'brandUser', 'handledBy', 'messages' => fn($query) => $query->latest()->limit(1)])
		...
		$moderatorName = $conversation->handledBy?->name;
		$influencerNameForModerator = $conversation->influencer->user->name;
	...
	<div
		class="h-[calc(100vh-120px)] rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900 overflow-hidden transition-all duration-200">
		<div class="flex h-full w-full">
			<!-- Sidebar -->
			<aside
				class="w-80 flex-shrink-0 border-r border-gray-200 dark:border-gray-800 bg-gray-50/40 dark:bg-gray-900/50 flex flex-col h-full">
				<header
					class="flex-shrink-0 border-b border-gray-200 bg-white/80 backdrop-blur-sm px-5 py-4 dark:border-gray-800 dark:bg-gray-900/80">
					<div class="flex items-center justify-between">
						<h2 class="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
							...
							Conversations
							<span
								class="text-xs font-normal text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">{{ $sidebarConversations->count() }}</span>
						</h2>
					...
					@forelse ($sidebarConversations as $item)
						...
						<a href="{{ route('dashboard.conversations.show', $item) }}"
							...
							<div class="relative shrink-0">
								<div
									class="h-12 w-12 rounded-full overflow-hidden bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 flex items-center justify-center text-sm font-semibold text-indigo-700 dark:text-indigo-300 shadow-sm">
									@if ($partyAvatar)
								...
								<span
									class="absolute bottom-0 right-0 block h-3 w-3 rounded-full bg-green-500 ring-2 ring-white dark:ring-gray-900"></span>
							</div>
							<div class="min-w-0 flex-1">
								<div class="flex items-center justify-between gap-2">
									<h3 class="truncate text-sm font-semibold text-gray-900 dark:text-white">{{ $partyName }}</h3>
									<span
										class="text-[11px] font-medium text-gray-400 dark:text-gray-500">{{ $item->updated_at->diffForHumans() }}</span>
								</div>
			...
			<section class="flex-1 flex flex-col h-full bg-white dark:bg-gray-900 min-w-0">
				...
				<div
					class="flex-shrink-0 border-b border-gray-200 bg-white/80 backdrop-blur-sm px-5 py-3 dark:border-gray-800 dark:bg-gray-900/80">
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3 min-w-0 flex-1">
							<button
								class="lg:hidden p-1.5 -ml-1 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
								<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 6h16M4 12h16M4 18h7" />
								</svg>
							</button>
							<div
								class="h-10 w-10 rounded-full overflow-hidden bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 flex items-center justify-center flex-shrink-0">
						...
						@if ($conversation->order_id)
							<button id="toggleOrderDetails"
								class="p-2 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition flex-shrink-0"
				...
				@if ($conversation->order_id)
					<div id="orderDetailsPanel"
						class="flex-shrink-0 border-b border-gray-200 dark:border-gray-800 bg-gradient-to-r from-indigo-50/50 to-purple-50/50 dark:from-indigo-950/20 dark:to-purple-950/20 hidden transition-all duration-300">
						<div class="p-4">
							<div class="flex items-center justify-between mb-3">
								...
								<a href="{{ route('dashboard.orders.show', $conversation->order_id) }}"
							...
							<div class="grid grid-cols-2 md:grid-cols-3 gap-3">
								...
								<div class="bg-white/60 dark:bg-gray-800/40 rounded-lg px-3 py-2 backdrop-blur-sm">
									...
									<p class="text-sm font-semibold text-gray-900 dark:text-white">
										${{ number_format($conversation->order?->total_amount ?? 0, 2) }}</p>
								</div>
								<div class="bg-white/60 dark:bg-gray-800/40 rounded-lg px-3 py-2 backdrop-blur-sm">
									<p class="text-[10px] uppercase tracking-wider text-gray-500 dark:text-gray-400">Status</p>
									<span
										class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-full {{ $getOrderStatusColor($conversation->order?->status ?? 'pending') }}">
				...
				@if (auth()->user()->user_type === 'admin' && !$conversation->handled_by_user_id)
					<div
						class="flex-shrink-0 border-b border-yellow-200 dark:border-yellow-900/30 bg-yellow-50/50 dark:bg-yellow-900/20 p-4 transition-all duration-300">
						<div class="flex items-center justify-between">
							<div>
								<h4 class="font-semibold text-yellow-900 dark:text-yellow-100 flex items-center gap-2">
									<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
											d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM9 19c-4.3 0-8 1.343-8 3v2h16v-2c0-1.657-3.7-3-8-3z" />
									</svg>
									Assign Moderator
								</h4>
								<p class="mt-1 text-xs text-yellow-800 dark:text-yellow-300">This conversation needs a moderator to handle
									influencer responses.</p>
							</div>
						</div>
				...
				@elseif (auth()->user()->user_type === 'admin' && $conversation->handled_by_user_id)
					<div
						class="flex-shrink-0 border-b border-green-200 dark:border-green-900/30 bg-green-50/50 dark:bg-green-900/20 p-4">
						<div class="flex items-center justify-between">
							<div>
								...
								<p class="mt-1 text-sm text-green-800 dark:text-green-300">
									<a href="{{ route('dashboard.moderators.show', $conversation->handledBy) }}" class="font-medium text-green-700 dark:text-green-200 hover:underline">{{ $conversation->handledBy->name }}</a> is handling this conversation as <a href="{{ route('dashboard.influencers.view', $conversation->influencer) }}" class="font-medium text-green-700 dark:text-green-200 hover:underline">{{ $conversation->influencer->user->name }}</a>
								</p>
							</div>
						</div>
					</div>
				...
				<div id="messagesContainer"
					class="flex-1 overflow-y-auto p-5 space-y-4 bg-gradient-to-b from-gray-50 to-white dark:from-gray-950/30 dark:to-gray-900">
					@forelse ($messages as $message)
						@php
							$isOwn = $message->sender_user_id === auth()->id();
							$senderName = $message->sender?->name ?? 'Unknown';
							$senderAvatar = filled($message->sender?->profile_image_path ?? null)
							    ? image_url($message->sender->profile_image_path)
							    : null;
						...
						<div class="flex {{ $isOwn ? 'justify-end' : 'justify-start' }} animate-fade-in">
							<div class="flex max-w-[85%] md:max-w-[75%] items-end gap-2 {{ $isOwn ? 'flex-row-reverse' : '' }}">
								@if (!$isOwn)
									<div
										class="h-8 w-8 shrink-0 rounded-full overflow-hidden bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 flex items-center justify-center text-xs font-semibold text-indigo-700">
										@if ($senderAvatar)
								...
								<div class="group relative">
									<div
										class="rounded-2xl px-4 py-2.5 shadow-sm transition-all duration-150
                                        {{ $isOwn
									...
									<div
										class="mt-1 flex items-center gap-1.5 text-[10px] font-medium px-1
				...
				<div
					class="flex-shrink-0 border-t border-gray-200 bg-white/80 backdrop-blur-sm p-4 dark:border-gray-800 dark:bg-gray-900/80">
					<form action="{{ route('dashboard.conversations.storeMessage', $conversation) }}" method="POST"
						class="flex gap-2">
						@csrf
						<div class="flex-1 relative">
							<input type="text" name="message" placeholder="Write your message..." autocomplete="off"
							class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:focus:ring-indigo-900/50 transition">
						</div>
						<button type="submit"
