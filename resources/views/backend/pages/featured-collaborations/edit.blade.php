	...
	<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-6">
		<div class=" mx-auto">
			...
			<div class="mb-8">
				...
				<h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Collaboration</h1>
			</div>

			<!-- Form Card -->
			<div
				class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
				<form action="{{ route('dashboard.featured-collaborations.update', $featuredCollaboration) }}" method="POST"
					...
					<div class="mb-6">
						...
						<input type="text" id="brand_name" name="brand_name"
							value="{{ old('brand_name', $featuredCollaboration->brand_name) }}"
					...
					<div class="mb-6">
						...
						<select id="asset_type" name="asset_type"
							...
							<option value="image" {{ old('asset_type', $featuredCollaboration->asset_type) === 'image' ? 'selected' : '' }}>
								Image</option>
							<option value="video" {{ old('asset_type', $featuredCollaboration->asset_type) === 'video' ? 'selected' : '' }}>
								Video</option>
						</select>
						@error('asset_type')
							<p class="text-red-500 dark:text-red-400 text-xs mt-1">{{ $message }}</p>
					...
					<div class="mb-6">
						...
						<input type="number" id="sort_order" name="sort_order"
							value="{{ old('sort_order', $featuredCollaboration->sort_order) }}" min="0"
							class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:border-transparent transition-all duration-200"
							placeholder="0">
						@error('sort_order')
							<p class="text-red-500 dark:text-red-400 text-xs mt-1">{{ $message }}</p>
					...
					<div class="mb-8">
						<label class="flex items-center gap-3 cursor-pointer group">
							<input type="checkbox" name="is_published" value="1"
								{{ old('is_published', $featuredCollaboration->is_published) ? 'checked' : '' }}
