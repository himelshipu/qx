...
@section('content')
	<x-backend.shell.breadcrumb :links="[['label' => 'Featured Collaborations', 'url' => route('dashboard.featured-collaborations.index')]]" pageTitle="Edit Collaboration" />

	<div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
		<form id="featured-collaboration-form" action="{{ route('dashboard.featured-collaborations.update', $featuredCollaboration) }}" method="POST" enctype="multipart/form-data" class="p-6">
			@csrf
			@method('PATCH')

			@include('backend.pages.featured-collaborations._form', ['featuredCollaboration' => $featuredCollaboration])
