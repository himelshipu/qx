...
@section('content')
	...
	<div class="space-y-6">
		...
		<x-sortable-list
			:items="$brands"
			modelName="Brand"
			reorderRoute="{{ route('dashboard.brands.reorder') }}"
			editRoute="dashboard.brands.edit"
			title="Reorder Brands"
			description="Drag brands to reorder them. The order determines how they appear across the platform."
		/>
							...
							@forelse ($brands as $brand)
								...
								<tr class="transition hover:bg-gray-50/70 dark:hover:bg-gray-800/40">
									<td class="px-4 py-3">
										<div class="flex items-center gap-3">
											<div
												...
												@if ($previewUrl)
													<img src="{{ $previewUrl }}" alt="{{ $brand->brand_name }}" class="h-10 w-10 object-cover">
												@else
													<span
														class="text-sm font-semibold text-gray-500 dark:text-gray-300">{{ strtoupper(substr($brand->brand_name, 0, 1)) }}</span>
												@endif
											</div>
											<div>
									...
									<td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
										{{ $brand->user?->name ?? 'Unknown' }}
									</td>
									<td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
										{{ $brand->user?->email ?? ($brand->email ?? 'N/A') }}
									</td>
									<td class="px-4 py-3">
										<div class="text-xs text-gray-600 dark:text-gray-300">
											<span class="inline-flex rounded bg-gray-100 px-2 py-1 dark:bg-gray-800">Orders:
												{{ $brand->orders_count }}</span>
											<span class="inline-flex rounded bg-gray-100 px-2 py-1 dark:bg-gray-800">Reviews:
									...
									<td class="px-4 py-3">
										<div class="flex items-center justify-end gap-2">
											<a href="{{ route('dashboard.brands.view', $brand) }}"
											...
											<form action="{{ route('dashboard.brands.destroy', $brand) }}" method="POST">
												...
												<button type="submit"
													...
													data-confirm-message="Delete this brand? This action cannot be undone."
