...
@section('content')
	...
	<div class="space-y-6" id="brands-dashboard"
		data-csrf-token="{{ csrf_token() }}"
		data-filter-results-route="{{ route('dashboard.brands.table') }}"
		data-max-featured="{{ (int) config('brand.max_featured', 20) }}"
		data-status-toggle-template="{{ route('dashboard.brands.toggle-status', ['brand' => '__ID__']) }}"
		data-featured-add-template="{{ route('dashboard.brands-featured.add', ['brand' => '__ID__']) }}"
		data-featured-remove-template="{{ route('dashboard.brands-featured.remove', ['brand' => '__ID__']) }}"
		data-featured-list-route="{{ route('dashboard.brands-featured.list') }}"
		data-featured-search-route="{{ route('dashboard.brands-featured.search') }}"
		data-featured-reorder-route="{{ route('dashboard.brands-featured.reorder') }}">
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
			...
			<div class="rounded-xl border border-indigo-200 bg-indigo-50 p-4 dark:border-indigo-900/40 dark:bg-indigo-900/20">
				<p class="text-xs font-semibold uppercase tracking-wider text-indigo-700 dark:text-indigo-300">Verified</p>
				<p class="mt-2 text-2xl font-semibold text-indigo-700 dark:text-indigo-200">{{ $stats['verified'] }}</p>
			</div>
		</div>
		...
		<div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
			<div
				...
				<div>
					<h3 class="text-lg font-semibold text-gray-900 dark:text-white">Brand Management</h3>
					<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage brand accounts, status, and profile quality from one
						view.</p>
				</div>
			...
			<div class="p-5">
					...
					<div class="md:col-span-3">
						<div class="relative">
							...
							<input id="q" name="q" type="text" value="{{ $search }}"
								placeholder="Search by brand, contact, email, city, or country"
				...
				@include('backend.pages.brands._results', ['brands' => $brands])
			</div>
		</div>
	</div>
		...
		<div class="flex min-h-screen items-center justify-center p-4">
			<div class="relative flex max-h-[60vh] w-full max-w-4xl flex-col overflow-hidden rounded-lg bg-white shadow-xl dark:bg-gray-900">
				<div class="flex items-center justify-between border-b border-gray-200 p-6 dark:border-gray-800">
					<h2 class="text-xl font-semibold text-gray-900 dark:text-white">Manage Featured Brands</h2>
				...
				<div class="flex-1 overflow-y-auto p-6">
					<div class="mb-6">
						<div class="relative">
							...
							<input id="featured-search" type="text" placeholder="Search brands to add..."
								class="h-10 w-full rounded-lg border border-gray-200 bg-transparent pl-10 pr-3 text-sm text-gray-900 focus:border-gray-400 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">
						</div>
					...
					<div class="mb-4">
						<h3 class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
							Featured Brands (<span id="featured-count">0</span>/{{ (int) config('brand.max_featured', 20) }})
						</h3>
