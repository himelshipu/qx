...
@section('content')
	<x-backend.shell.breadcrumb pageTitle="Edit Brand" />
	...
	<div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
		<div
			...
			<div>
				<h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit: {{ $brand->brand_name }}</h3>
				<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update account information, contact details, and brand
					assets.</p>
			</div>
		...
		<form action="{{ route('dashboard.brands.update', $brand) }}" method="POST" enctype="multipart/form-data" novalidate
			class="space-y-6 p-5">
			@csrf
			@method('PUT')

			@include('backend.pages.brands._form', ['brand' => $brand])
