...
@section('content')
	...
	<div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
		<div
			...
			<div>
				<h3 class="text-lg font-semibold text-gray-900 dark:text-white">Edit:
					{{ $influencer->display_name ?: $influencer->user?->name }}</h3>
		...
		<form action="{{ route('dashboard.influencers.update', $influencer) }}" method="POST" enctype="multipart/form-data" novalidate
			class="space-y-6 p-5">
			@csrf
			@method('PUT')
