const ROOT_SELECTOR = "#blog-dashboard";
const FILTER_FORM_SELECTOR = "#blog-filters-form";
const RESULTS_SELECTOR = "#blog-results";
...
const initBlogDashboard = () => {
	...
	const featuredTemplate = root.dataset.featuredToggleTemplate;
	...
	const bindFeaturedToggles = () => {
		root.querySelectorAll("[data-featured-toggle]").forEach((toggle) => {
			toggle.addEventListener("change", async () => {
				const id = toggle.dataset.id;
				const prev = !toggle.checked;
				try {
					const response = await fetch(featuredTemplate.replace("__ID__", id), {
						method: "POST",
						headers: {
							"X-CSRF-TOKEN": csrfToken,
							"X-Requested-With": "XMLHttpRequest",
...
if (document.readyState === "loading") {
	document.addEventListener("DOMContentLoaded", initBlogDashboard);
} else {
	initBlogDashboard();
}