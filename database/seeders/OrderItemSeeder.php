...
class OrderItemSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $packagesByCreator = DB::table('packages')->get()->groupBy('creator_id');
        $orderItemStatuses = ['pending', 'accepted', 'in_progress', 'delivered', 'approved', 'rejected', 'cancelled'];

        foreach ($orders as $order) {
            $creatorId = $order->accepted_for_creator_id ?: DB::table('creators')->inRandomOrder()->value('id');
            if (!$creatorId) {
                continue;
            }

            $packageId = ($packagesByCreator[$creatorId] ?? collect())->random()->id ?? null;
